...
public class ZabbixUtil {
    ...
    public static String createItem(String name, String hostId, String key) {
       /* Map<String, Object> params = new HashMap<>();
        params.put("delay ", "10s");
        params.put("name", name);
        params.put("hostid", hostId);
        params.put("value_type", 3);
        params.put("interfaceid", getHostinterface(hostId));
        Map requestMap = getRequestMap("item.create", params);
        ((Map)requestMap.get("params")).put("key_", key);
        String s = sendPost(requestMap);
    ...
    public static void createVirtualHost() {
        ...
        if (fms5000 != null && fms5000.size() > 0) {
            List<String> fms5000HotsIdList = creatHost(fms5000.size(), "127.0.0.1", "FMS5000", zabbix_servers_groupid);
            //  创建item
...
//        getDashboard(Arrays.asList(new String[]{"9"}));
        createVirtualHost();
    ...
    public static void createDashboard(List<String> groupIds, List<Map<String, String>> garphidAndName) {
      /*  Map<String, Object> params = new HashMap<>();
        ...
        try {
            FileInputStream oFile = new FileInputStream("dashboard.properties");
            prop.load(oFile);
...
//                    newWidgets.add(widget);
            ...
            map.put("name", "java create dashboard");


            deleteDashboard("java create dashboard");
...
//        params.put("dashboardids", new ArrayList<>());
    ...
    public static void getDashboard(List groupids) {
        ...
        try {
            FileOutputStream oFile = new FileOutputStream("dashboard.properties", false);//true表示追加打开
    ...
    public static Map<String, List<String>> getGroupName() {
        ...
        if (result.size() > 0) {
            for (int i = 0; i < result.size(); i++) {
                ...
                if (!StringUtils.isEmpty(name) && name.startsWith("FMS5000")) {
                    fms.add(name);
                }
    ...
    static class ZabbixColor {
        ...
        public static String randomColor() {
            java.util.List<Integer> random = new ArrayList();
            while (random.size() < 3) {
